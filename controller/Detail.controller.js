			...
			'zfisncv2.controller.Detail', {
				...
				onListUpdateFinished: function(oEvent) {
					...
					if (
						...
						if (iTotalItems) {
							sTitle = this.getResourceBundle().getText('detailLineItemTableHeadingCount', [
								iTotalItems
							]);
						} else {
				...
				onListUpdateFinishedLog: function(oEvent) {
					...
					// only update the counter if the length is final
					if (
						this.byId('lineItemsListLog').getBinding('items').isLengthFinal()
					) {
						if (iTotalItems) {
							sTitle = this.getResourceBundle().getText('detailLineItemTableHeadingCountLog', [
								iTotalItems
							]);
						} else {
							//Display 'Line Items' instead of 'Line items (0)'
				...
				onSearchBelnr: function(oEvent) {
					...
					if (sQuery) {
						aFilter.push(
							new Filter('bktxt', sap.ui.model.FilterOperator.Contains, sQuery)
						);
						aFilter.push(
							new Filter('sgtxt', sap.ui.model.FilterOperator.Contains, sQuery)
						);
						aFilter.push(
							new Filter('xblnr', sap.ui.model.FilterOperator.Contains, sQuery)
						);
						aFilter.push(
							new Filter('blart', sap.ui.model.FilterOperator.Contains, sQuery)
						);
						aFilter.push(
							new Filter('mwskz', sap.ui.model.FilterOperator.Contains, sQuery)
						);
						aFilter.push(
							new Filter('belnr', sap.ui.model.FilterOperator.Contains, sQuery)
						);
					} // filter binding
				...
				*/
				...
				onBtnFc: function(oEvent) {
					...
					for (var i = 0; i < aSelectedData.length; i++) {
						...
						for (var j = 0; j < aLogisticData.length; j++) {
							...
							if (aLogisticData[j].belnr === aSelectedData[i].belnr) {
				...
				addItems: function() {
					//var aLogisticData = this.getModel("LogisticData").getData().Items;
					var aLogisticData = this.getModel('oModelLog').getData().results;
					var aSelectedDataPath = this.getView().byId('lineItemsListLog')._aSelectedPaths;
					var aSelectedLogisticData = [];
					...
					var vTotC1 = 0;
					var vTotC0 = 0;
					//Agregamos las posiciones que fueron seleccionadas a partir del path
					for (var i = 0; i < aSelectedDataPath.length; i++) {
						var dataPath = aSelectedDataPath[i];
						var dataIndex = dataPath.split('/')[2];
						aSelectedLogisticData.push(aLogisticData[dataIndex]);
					}
					/*for (var j = 0; j < aLogisticData.length; j++) {
				if (aLogisticData[j].mwskz === 'C2') {
					...
					for (var j = 0; j < aSelectedLogisticData.length; j++) {
						if (aSelectedLogisticData[j].mwskz === 'C2') {
							vTotC2 = vTotC2 + parseFloat(aSelectedLogisticData[j].valor_nc);
						} else if (aSelectedLogisticData[j].mwskz === 'C1') {
							vTotC1 = vTotC1 + parseFloat(aSelectedLogisticData[j].valor_nc);
						} else if (aSelectedLogisticData[j].mwskz === 'C0') {
							vTotC0 = vTotC0 + parseFloat(aSelectedLogisticData[j].valor_nc);
						}
					} //Inicializar items data para el K4
					this._updateItemsData('ItemsData', null, true);
					if (vTotC2 !== 0) {
				...
				onBtnContab: function() {
					...
					oModel.read(
						bPath, {
							success: function(oData) {
								if (!oData.Block) {
									//Quitar espacios en blanco de la referencia
									//var referencia = oHeaderData.getProperty("/xblnr").trim();
									var referencia = oHeaderData.getProperty('/xblnr');
									//Validacion de la referencia
									if (referencia.length === 15) { //Longitud de 15 caracteres
										//Verificar los guiones en el formato
										if (referencia.charAt(3) === '-' && referencia.charAt(7) === '-') {
											//Referencia correcta
										} else {
											//mensaje de error
											sap.m.MessageBox.error('La referencia no cumple el formato XXX-XXX-XXXXXXX');
											return;
										}
									} else {
										//Mensaje de error
				...
				fnCallImports: function() {
					...
					for (var j = 0; j < oModelLog.length; j++) {
						...
						if (oModelLog[j].mwskz === 'C1') {
							...
							vTotSinIvaNC = vTotSinIvaNC + (parseFloat(oModelLog[j].valor_nc) / 1.1);
						} else {
										...
										bPath, {
											success: function(oData, oResponse) {
												if (oData.Autho) {
													...
													sap.m.MessageBox.error(
														'ERROR al procesar NC de Proveedor, No cuenta con autorizacion para facturar'
													);
												}
											}
										}
								...
								case 'Continuar':
									...
									setTimeout(
										function() {
											...
											sap.m.MessageBox.error(
												'Error al crear documento: Error de conexion, revisar conexion a la red e intente de nuevo.'
											);
											//Se detiene el proceso
											return;
										},
										300000
									); //Tiempo de mas o menos 5 minutos
									break;
							} //
				...
				onVendorAmountChange: function(oEvent) {
					...
					//var iTotalItems = this._calcularTotalSeleccionadoLog();
					var oControlModel = this.getModel('controlModel');
				...
				_kostlValueHelpSearch: function(oEvent) {
					...
					var sValue = oEvent.getParameter('value');
					aFilters.push(
						new Filter('kostl', sap.ui.model.FilterOperator.Contains, sValue)
					);
					aFilters.push(
						new Filter('ktext', sap.ui.model.FilterOperator.Contains, sValue)
					);
				...
				_hktidValueHelpSearch: function(oEvent) {
					...
					var sValue = oEvent.getParameter('value');
					aFilters.push(
						new Filter('hktid', sap.ui.model.FilterOperator.Contains, sValue)
					);
					var vHbkid = this.getView().byId('id_ihbkid').getValue();
					aFilters.push(
						new Filter('hbkid', sap.ui.model.FilterOperator.Contains, vHbkid)
					);
					oEvent.getSource().getBinding('items').filter(aFilters);
				},
				...
				_segmentValueHelpSearch: function(oEvent) {
					...
					var sValue = oEvent.getParameter('value');
					aFilters.push(
						new Filter('saknr', sap.ui.model.FilterOperator.Contains, sValue)
					);
					aFilters.push(
						new Filter('txt20', sap.ui.model.FilterOperator.Contains, sValue)
					);
					oEvent.getSource().getBinding('items').filter(aFilters);
				},
				//Segment Close
				...
				_checkObligatoryFields: function() {
					...
					var sBktxt = oHeaderData.getProperty('/bktxt');
					if (!sBktxt) {
						return 'Texto de Cabebera vacío';
					}
					return 'OK';
				},
				...
				_updateLogisticData: function(sPath, data, bDelete) {
					...
					var oData = oModel.getData();
					//var oData = this.getModel("oModelLog").getData();
					var aData = oData.Items;
					...
					if (sPath && bDelete) {
						if (sPath === 'LogisticData') {
							...
							//Delete entry
							//oData = oData = this.getModel("oModelLog").getData();
							//aData = oData.results;
							for (i = 0; i < aData.length; i++) {
								if (aData[i] != null) {
				...
				_calcularTotalItems: function() {
					...
					var aDataItems = oDataItems.Items;
					var iTotal = 0;
					//var iTotal = aDataItems[aDataItems.length - 1].dmbtr * 1;
					var iAux = 0;
				...
				_armarDocsLogistic: function() {
					var aPosLogist = [];
					var aLogisticData1 = this.getModel('LogisticData').getData().Items;
					var aLogisticData = this.getModel('oModelLog').getData().results;
					var aSelectedDataPath = this.getView().byId('lineItemsListLog')._aSelectedPaths;
					var aSelectedLogisticData = [];
					aLogisticData.sort();
					aSelectedDataPath.sort();
					//Agregamos las posiciones que fueron seleccionadas a partir del path
					for (var j = 0; j < aSelectedDataPath.length; j++) {
						var dataPath = aSelectedDataPath[j];
						var dataIndex = dataPath.split('/')[2];
						var dataItem = aLogisticData[dataIndex];
						if (dataItem.valor_nc === 0) {
							aSelectedLogisticData.push(aLogisticData1[j]);
						} else {
							aSelectedLogisticData.push(dataItem);
						}
					}
					for (var i = 0; i < aSelectedLogisticData.length; i++) {
						var logistic = {};
						logistic.bukrs = aSelectedLogisticData[i].bukrs;
						logistic.belnr = aSelectedLogisticData[i].belnr;
						logistic.gjahr = aSelectedLogisticData[i].gjahr;
						logistic.buzei = aSelectedLogisticData[i].buzei;
						logistic.ean11 = aSelectedLogisticData[i].ean11;
						logistic.bpmng = aSelectedLogisticData[i].bpmng;
						logistic.bprme = aSelectedLogisticData[i].bprme;
						logistic.txz01 = aSelectedLogisticData[i].txz01;
						logistic.netpr = aSelectedLogisticData[i].netpr;
						logistic.netwr = aSelectedLogisticData[i].netwr;
						logistic.menge = aSelectedLogisticData[i].menge;
						logistic.bstme = aSelectedLogisticData[i].bstme;
						logistic.mwskz = aSelectedLogisticData[i].mwskz;
						logistic.menge_nc = aSelectedLogisticData[i].menge_nc.toString();
						logistic.valor_nc = aSelectedLogisticData[i].valor_nc.toString();
						logistic.waers = aSelectedLogisticData[i].waers;
				...
				_onBindingChange: function() {
					...
					oViewModel.setProperty(
						'/saveAsTileTitle',
						oResourceBundle.getText('shareSaveTileAppTitle', [
							sObjectName
						])
					);
				...
				_onMetadataLoaded: function() {
					...
					oLineItemTable.attachEventOnce(
						'updateFinished',
						function() {
...
//# sourceURL=https://retsapgwdev01.ajvierci.com.py:8400/sap/bc/ui2/%3Cunknown%3E/zfisncv2/controller/Detail.controller.js?eval